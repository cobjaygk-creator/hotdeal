...
CATEGORIES = ["식품", "생활", "PC", "가전", "의류", "유아", "게임", "여행", "기타"]
...
_SELLER_RULES: list[tuple[tuple[str, ...], str]] = [
    ...
    (
        (
            "하나투어", "모두투어", "노랑풍선", "참좋은여행", "웹투어", "인터파크투어",
            "마이리얼트립", "트립닷컴", "trip.com", "야놀자", "여기어때", "아고다",
            "agoda", "익스피디아", "expedia", "클룩", "klook", "스카이스캐너",
            "skyscanner", "부킹닷컴", "booking.com", "에어비앤비", "airbnb", "트리플",
        ),
        "여행",
    ),
    (("다이소",), "생활"),
...
_KEYWORDS: dict[str, tuple[str, ...]] = {
    "게임": (
        ...
        # 2026-09-11 기타 오분류 실사례 반영 (프로덕션 246건 실측)
        "스팀월렛",
    ),
    "여행": (
        "패키지여행", "자유여행", "왕복항공권", "편도항공권", "항공권", "크루즈",
        "글램핑", "풀빌라", "호텔숙박권", "리조트이용권", "펜션숙박권",
        "게스트하우스", "여행자보험", "공항라운지", "패스트트랙", "면세점",
        "숙박권", "호캉스", "동남아여행", "일본여행", "유럽여행", "괌여행",
        "발리여행", "다낭", "나트랑", "후쿠오카", "오사카", "홋카이도",
        "몰디브", "칸쿤", "세부", "보라카이", "여행패키지", "전세기",
    ),
    "유아": (
    ...
    "PC": (
        ...
        # 2026-09-11 기타 오분류 실사례 반영 (프로덕션 246건 실측)
        "완본체", "그램북", "시놀로지", "나스용", "aorus", "gigabyte",
        "usb 메모리", "usb메모리", "보조배터리", "충전기", "충전케이블", "gan", "무선 이어폰", "gps", "속도계",
    ),
    "가전": (
        ...
        "인치 tv", "인치tv", "나노셀",
        # 2026-09-11 기타 오분류 실사례 반영 (프로덕션 246건 실측)
        "음식물처리기", "음식물 처리기", "바디트리머", "에어스무스", "스피커", "드라이기", "고데기", "tv보호기", "보호기",
    ),
    "의류": (
    ...
    "식품": (
        ...
        # 2026-09-11 기타 오분류 실사례 반영 (프로덕션 246건 실측)
        "청국장", "유과", "치즈볼", "마라탕", "명이나물", "더단백",
        "배스킨라빈스", "소세지", "mintia", "불고기", "칼국수", "칠리소스",
        "파워에이드", "아이시스", "교자", "오로나민", "강정", "토마토",
        "쌍화", "크림치즈", "누룽지", "치즈", "아이스티", "설렁탕", "약과",
        "카스텔라", "할루미", "햇밤", "도시락", "덮밥", "간편식", "가정간편식", "hmr", "단호박", "굴소스", "복숭아", "해장국", "꽃게", "연어", "순대", "곤약젤리", "오코텍스", "김", "소스",
    ),
    "생활": (
        ...
        "냄비받침", "전기요", "덤벨", "푸쉬업바",
        # 2026-09-11 기타 오분류 실사례 반영 (프로덕션 246건 실측)
        "워셔액", "여성청결제", "수저", "앙블랑", "스팟패치", "면봉",
        "모션데스크", "본드", "장작", "과탄산소다", "도마",
        "아이깨끗해", "펫푸드", "마스크", "렌즈와이프", "물걸레청소포", "청소포", "매트리스커버", "방수커버", "낚싯대", "공구", "파라솔",
    ),
}

_PRIORITY = ("게임", "유아", "여행", "PC", "가전", "의류", "식품", "생활")
...
_HEURISTICS: list[tuple[re.Pattern[str], str]] = [
    ...
    # 2026-09-11: "LG 그램 14 ..." 처럼 "그램북"이 아닌 형태 (바지락과 겹치는
    # "바지"는 여기서 부정형 전방탐색으로 안전하게)
    (re.compile(r"lg\s*그램"), "PC"),
    (re.compile(r"바지(?!락)"), "의류"),
    (re.compile(r"웍(?!스)"), "생활"),  # "…웍" 조리도구 vs "시스템웍스" 같은 회사명 오탐 방지
    ...
    (re.compile(r"(\d+\s*박\s*\d+\s*일|왕복항공권|편도항공권|패키지여행|자유여행|호텔숙박권|리조트이용권|크루즈여행|글램핑|풀빌라)"), "여행"),
