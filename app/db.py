...
async def _ensure_auth_tables(conn: aiosqlite.Connection) -> None:
    ...
    cur = await conn.execute("PRAGMA table_info(deal_reports)")
    ...
    cur = await conn.execute("PRAGMA table_info(deals)")
    deal_cols = {row[1] for row in await cur.fetchall()}
    if "toss_review_status" not in deal_cols:
        await conn.execute("ALTER TABLE deals ADD COLUMN toss_review_status TEXT NOT NULL DEFAULT 'pending'")
    if "toss_review_reason" not in deal_cols:
        await conn.execute("ALTER TABLE deals ADD COLUMN toss_review_reason TEXT")
...
async def _ensure_settings_table(conn: aiosqlite.Connection) -> None:
    ...
    await conn.execute("CREATE TABLE IF NOT EXISTS affiliate_clicks (id INTEGER PRIMARY KEY AUTOINCREMENT, platform TEXT NOT NULL, product_id TEXT, created_at TEXT NOT NULL)")
...
async def _ensure_coupang_table(conn: aiosqlite.Connection) -> None:
    ...
    for name, definition in (("original_url", "TEXT"), ("affiliate_url", "TEXT"), ("link_status", "TEXT NOT NULL DEFAULT 'pending'"), ("link_failure_reason", "TEXT"), ("link_converted_at", "TEXT"), ("link_verified_at", "TEXT"), ("shipping_fee", "INTEGER"), ("coupon_text", "TEXT"), ("option_text", "TEXT"), ("quantity_text", "TEXT")):
